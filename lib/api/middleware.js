...
const authRateLimiter = new RateLimiterMemory({
  points: 5, // 5 requests
  duration: 60, // per 1 minute by IP (prevents credential stuffing)
});

// Very strict rate limiter for signup/login attempts
const signupRateLimiter = new RateLimiterMemory({
  points: 3, // 3 attempts
  duration: 900, // per 15 minutes by IP (prevents brute force)
});
 ...
 * @param {string} type - 'general', 'auth', or 'signup' (default: 'general')
 */
export function rateLimit(type = 'general') {
  return (handler) => async (req, res) => {
    try {
      ...
      await limiter.consume(req.socket.remoteAddress || req.headers['x-forwarded-for'] || 'unknown-ip');
      return handler(req, res);
    } catch (rej) {
