...
export function isVitalNormal(vitalType, value, customThresholds = null) {
  ...
  // Determine direction based on normal range limits
  const direction = value < threshold.minValue ? 'low' : 'high';

  // Check if it is within warning range
  const isWithinWarning = (direction === 'low' && value >= threshold.warningMin) || 
                          (direction === 'high' && value <= threshold.warningMax);
