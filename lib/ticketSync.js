...
export const getSupportTickets = (callback) => {
  try {
    const q = query(
...
export const addTicketMessage = async (ticketId, message) => {
  try {
    logger.debug('TicketSync: Adding message to ticket:', ticketId);
    logger.debug('TicketSync: Message data:', message);
    ...
    logger.debug('TicketSync: Found ticket in cache:', ticket);
    ...
    // Use the Firestore document ID for the update
    const firestoreId = ticket.firestoreId || ticketId;
    logger.debug('TicketSync: Using Firestore ID:', firestoreId);
    ...
    logger.debug('TicketSync: Formatted message data:', messageData);

    // Update in Firestore using the correct document ID
    const ticketRef = doc(db, 'supportTickets', firestoreId);
    const updatedMessages = [...(ticket.messages || []), messageData];

    logger.debug('TicketSync: Updating Firestore with messages:', updatedMessages);
    ...
    logger.debug('TicketSync: Firestore update successful');
    ...
    if (ticketIndex !== -1) {
      ...
      logger.debug('TicketSync: Updated local cache');
    }

    // Notify listeners
    notifyTicketListeners('updated', globalTicketCache);
    logger.debug('TicketSync: Notified listeners');
...
export const initializeTicketSync = () => {
  logger.debug('Initializing ticket synchronization...');

  // Set up real-time listener
  getSupportTickets((tickets) => {
    logger.debug('Tickets synchronized:', tickets.length);
  });
};
