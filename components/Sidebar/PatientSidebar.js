    ...
    <div className=" flex h-screen bg-gray-100 dark:bg-gray-900 pt-20">
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex h-full">
          <span
            onClick={SetOpen}
            className=" absolute text-white md:hidden px-4 py-2 bg-green-600"
          >
            <FaList size={18} />
          </span>

          <nav
            className={` md:flex md:w-60 w-16 h-full transition ease-in-out duration-300 bg-gray2 dark:bg-gray6 ${isOpen ? " absolute" : "hidden"}`}
          >
            <ul className="w-full flex flex-col mx-auto px-2 py-2">
              ...
              {menuItems.map(({ href, title, icon }) => (
                <li key={title}>
                  <Link href={href}>
                    <div
                      className={`sidebar-icon group ${router.asPath === href && "dark:bg-blue-500 bg-blue-500 text-white"}`}
                    >
                      {icon}
