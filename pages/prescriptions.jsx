...
export default function Prescriptions() {
    ...
    setTimeout(() => {
      if (editingId) {
        ...
        setPrescriptions(prescriptions.map(p => 
          p.id === editingId ? { ...formData, id: editingId, status: 'active' } : p
        ));
        setEditingId(null);
      } else {
  ...
  const generateReminders = (frequency) => {
    switch (frequency) {
      ...
      case 'Every 12 hours': return ['08:00', '20:00'];
              ...
              <div className={styles.formGrid}>
                ...
                {formData.startDate && formData.duration && (
                  <div className={styles.datePreview}>
                    <span>End Date: {calculateEndDate(formData.startDate, formData.duration)}</span>
                  </div>
                )}
              </div>
              ...
              {formData.frequency && (
                <div className={styles.reminderPreview}>
                  ...
                  <div className={styles.reminderTimes}>
                    {generateReminders(formData.frequency).map((time, index) => (
              ...
              {filteredPrescriptions.length > 0 ? (
                <div className={styles.prescriptionsList}>
                  {filteredPrescriptions.map((prescription) => (
                    <motion.div 
                      key={prescription.id}
                      className={`${styles.prescriptionCard} ${prescription.status === 'completed' ? styles.completed : ''}`}
                    ...
                    >
                      ...
                      <div className={styles.prescriptionDetails}>
                        ...
                        <div className={styles.detailItem}>
                          ...
                          <span className={styles.detailValue}>{prescription.frequency}</span>
                        </div>
                        <div className={styles.detailItem}>
                          <span className={styles.detailLabel}>Duration:</span>
                          <span className={styles.detailValue}>{prescription.duration} days</span>
                        </div>
                        <div className={styles.detailItem}>
                        ...
                        {prescription.notes && (
                          <div className={styles.detailItem}>
                            ...
                            <span className={styles.detailValue}>{prescription.notes}</span>
                          </div>
                        )}
                      </div>
                      
                      {prescription.status === 'active' && prescription.reminders && (
                        <div className={styles.remindersSection}>
                          <h4>Reminder Times:</h4>
                          <div className={styles.remindersList}>
                            {prescription.reminders.map((time, index) => (
                              <span key={index} className={styles.reminderTime}>
                                {time}
                              </span>
                            ))}
                          </div>
                ...
                >
                  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M19 8L18.1327 20.1425C18.0579 21.1891 17.187 22 16.1378 22H7.86224C6.81296 22 5.94208 21.1891 5.86732 20.1425L5 8M10 12V18M14 12V18M15 8V5C15 4.44772 14.5523 4 14 4H10C9.44772 4 9 4.44772 9 5V8M4 8H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
